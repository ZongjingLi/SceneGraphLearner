...
class Objects3dDataset(Dataset):
    def __init__(self, config, sidelength, depth_aug=True, multiview_aug=True, phase='train', stage=0, vis=False, test=False):
        ...
        filenames = json.load(open(root + "/objects3d/%s.json"%phase))
        files = [os.path.join(path, f) for f in json.load(open(root + "/objects3d/%s.json"%phase))]
    ...
    def get_item(self, index):
        ...
        for i in range(len(segs)):
            ...
            dp_np = geometry.lift(x.flatten()[seg_mask], y.flatten()[seg_mask], depths[i].flatten(), intrinsics[None, :, :], True)

            dp_np = torch.cat([dp_np[:,:3], torch.ones_like(dp_np[..., :1]), dp_np[:,3:]], dim=-1)
            dp_nps.append(dp_np)

        coord, voxel_bool, coord_color = pickle.load(open(self.root + "/objects3d/parse_dicts/%s_.pkl"%filename, "rb"))
