...
class Visualizer:
    ...
    @property
    def per_result(self):return not self.per_batch
    ...
    def __str__(self):return f"{self.name}-{self.dataset}"
        ...
        if isinstance(texts, list):
            ...
            for text in texts:
                if sum(len(_) for _ in currents) < Visualizer.MAX_WIDTH:
                    ...
                    currents = []
            if len(currents) > 0: all_parts.append(' '.join(currents))
            if len(all_parts) > max_lines: all_parts[max_lines:] = []
...
class VisualizerList:
    ...
    def __str__(self):return str(self.visualizers)